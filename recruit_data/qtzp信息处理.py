...
class QtzpHanld(object):
    def __init__(self):
        ...
        self.engine=create_engine('mysql+mysqlconnector://user:password@ip:port/data_analysis')
        self.DBsession=sessionmaker(bind=self.engine)
    def salary_top10(self,tbname,col):
        from models import BPrank,BCrank,BTrank,PSrank,CSrank,ITCrank,BCSrank,BPSrank
        #职位分类工资top10
        session=self.DBsession()
        self.hive_con.sql('use zp')
        data=self.hive_con.sql('select %s,mon_wa from %s' % (col,tbname)).toDF('work_lable','mon_wa')
        ...
        #城市分类工资top10
        data=self.hive_con.sql('select work_area,mon_wa from %s' % tbname ).toDF('work_area','salary')
...
if __name__=='__main__':
    caozuo=QtzpHanld()
    # caozuo.salary_top10(tbname='zp',col='work_lable')
